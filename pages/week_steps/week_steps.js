...
Page({
  ...
  onShow: function () {
    ...
    app.getUserInfo(function (userInfo) {
      ...
      for (var idx in sports_record) {
        ...
        new_steps_record[sports_record[idx].data] = sports_record[idx].steps;
      }
      console.log("new_sports_record ", new_sports_record);
      console.log("new_steps_record", new_steps_record);
  ...
  update_day_step: function (e) {

    console.log('update day_step ', this.data.year, ' ', this.data.month, ' ', this.data.day, ' ', this.data.update_sport);

    if (!this.data.realsteps || this.data.realsteps == 0) {
      wx.showModal({
        title: "请选择大于0的步数",
    ...
    tmpdata["day_step"] = this.data.realsteps;
  ...
  update_week_step: function (e) {

    console.log('update week_step ', this.data.year, ' ', this.data.month, ' ', this.data.day, ' ', this.data.update_sport);

    if (!this.data.realweeksteps || this.data.realweeksteps == 0) {
      wx.showModal({
        title: "请选择大于0的步数",
        success: function (res) {
          if (res.confirm) {
    ...
    var totalsteps = this.data.realweeksteps * 7;
    if (app.globalData.user_info.prepoints > totalsteps) {
      wx.showModal({
        title: "请选择更大的平均步数",
        success: function (res) {
          if (res.confirm) {
    ...
    var diff = (parseInt(this.data.realweeksteps * 7) - parseInt(app.globalData.user_info.prepoints / 7) * 7) / 7;
    console.log("diff ", diff);

    var has_failed = false;
    for (var idx = 1; idx <= 7; ++idx) {

      var tmpdate = datenow.getFullYear().toString();
      if (datenow.getMonth() + 1 < 10)
        tmpdate += "0";
      tmpdate += (datenow.getMonth() + 1).toString();
      if (datenow.getDate() < 10)
        tmpdate += "0";
      tmpdate += datenow.getDate().toString();
      ...
      tmpdata["day_step"] = diff;

      if (this.data.steps_record[tmpdate] != undefined) {
        tmpdata["day_step"] = parseInt(this.data.steps_record[tmpdate]) + parseInt(tmpdata["day_step"]);
      }
      ...
      wx.request({
        ...
        success: function (res) {
          console.log("set_user_sports_info res ", res);
          console.log("++++++ ", parseInt(res.data.retcode), idx, has_failed);
          if (parseInt(res.data.retcode) == 0) {
  ...
  change_4_input: function (e) {
    this.data.realsteps = e.detail.value;
  },
  change_4_input_week: function (e) {
    this.data.realweeksteps = e.detail.value;
  }
})
